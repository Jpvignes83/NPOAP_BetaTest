...
class HOPSWindow:
    ...
    def _call_in_hops_cwd(self, func):
        """
        Exécute un callback HOPS dans le répertoire de travail HOPS
        puis restaure systématiquement le cwd global (isolation NPOAP).
        """
        if func is None:
            return
        prev_cwd = None
        ...
        try:
            ...
            prev_cwd = None
        try:
            target_cwd = self.log.get_param('directory')
        except Exception:
            target_cwd = None
        ...
        if function:
            ...
            def nested_after(function1, function2):
                ...
                def xx():
                    if not self._root_alive():
                        return
                    try:
                        self._call_in_hops_cwd(function1)
                    except Exception as e:
                        self.log.runtime_exception('after(nested) failed in window {0}'.format(self.name), e)
                        return
                    if self._root_alive():
            ...
            def internal_command():
                ...
                if isinstance(function, list):
                    ...
                    try:
                        self._call_in_hops_cwd(function)
                    except Exception as e:
                        self.log.runtime_exception('after() callback failed in window {0}'.format(self.name), e)
                        return
    ...
    def show(self):
        ...
        try:
            ...
            return
        try:
            self.log.runtime('Window shown: {0}'.format(self.name))
        except Exception:
            pass
        try:
    ...
    def run(self, f_after=None, f_before=None):
        ...
        try:
            self.log.runtime('Window start: {0}'.format(self.name))
        except Exception:
            pass
    ...
    def def_close(self):
        try:
            self.log.runtime('Window closing: {0}'.format(self.name))
        except Exception:
            pass
    ...
    def askdirectory(self):
        try:
            self.log.runtime('Dialog askdirectory opened from {0}'.format(self.name))
        except Exception:
            pass
        value = askdirectory()
        try:
            self.log.runtime('Dialog askdirectory result from {0}: {1}'.format(self.name, value))
        except Exception:
            pass
        return value
    ...
    def askyesno(self, *args, **kwargs):
        if self._root_alive():
            try:
                ...
                pass
        value = askyesno(*args, **kwargs)
        try:
            self.log.runtime('Dialog askyesno result from {0}: {1}'.format(self.name, value))
        except Exception:
            pass
        return value
    ...
    def askyesnocancel(self, *args, **kwargs):
        if self._root_alive():
            try:
                ...
                pass
        value = askyesnocancel(*args, **kwargs)
        try:
            self.log.runtime('Dialog askyesnocancel result from {0}: {1}'.format(self.name, value))
        except Exception:
            pass
        return value
    ...
    def showinfo(self, *args, **kwargs):
        try:
            ...
            self.log.runtime('Dialog showinfo from {0}: {1} | {2}'.format(self.name, title, msg))
        except Exception:
            pass
        if self._root_alive():
            try:
...
class HOPSButton(HOPSWidget):
    ...
    def internal_command(self):
        try:
            self.log.runtime('Button click [{0}] in window {1}'.format(self.variable.get(), self.window.name))
        except Exception:
            pass
        ...
        try:
            if isinstance(self.command, list):
                ...
                self.window._call_in_hops_cwd(self.command)
        except Exception as e:
            self.log.runtime_exception(
                'Button command failed [{0}] in window {1}'.format(self.variable.get(), self.window.name), e
            )
            raise
...
class HOPSDropDown(HOPSWidget):
    ...
    def __init__(self, window, initial, options, instance, command=None, width=None):
        ...
        if width:
            ...
            widget = Combobox(window.main_frame, textvariable=self.variable, state='readonly',
                              width=int(window.log.entries_width * 0.8), style=style_name)
