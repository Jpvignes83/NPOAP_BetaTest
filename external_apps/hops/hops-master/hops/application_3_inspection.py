...
def _robust_z(x, invert=False):
    """Score robuste type MAD ; invert=True si une valeur plus grande est meilleure (ex. SNR)."""
    x = np.asarray(x, dtype=float)
    med = np.median(x)
    mad = np.median(np.abs(x - med))
    scale = 1.4826 * max(mad, 1e-30)
    z = (x - med) / scale
...
class InspectiontWindow(MainWindow):
    ...
    def __init__(self, log):
        ...
        self.sky_std_array = np.array(self.sky_std_array)
        ...
        self._build_quality_criteria_file_and_worst_mask()
        ##############################################################################
        ...
        self.inspection_figure_ax1.plot(self.time_array, self.sky_mean_array, 'ko', ms=3, label='Frames', zorder=3)
        self.excluded1,  = self.inspection_figure_ax1.plot(self.time_array[np.where(self.skip_array)], self.sky_mean_array[np.where(self.skip_array)], 'ro', ms=3, zorder=4)
        self.lowq1, = self.inspection_figure_ax1.plot(
            self.time_array[self.worst_quality_mask], self.sky_mean_array[self.worst_quality_mask],
            's', ms=6, linestyle='None', color='red', markerfacecolor='none', markeredgewidth=1.8,
            markeredgecolor='red', zorder=5, label='Worst 10% quality')
        ...
        self.inspection_figure_ax2.plot(self.time_array, self.psf_array, 'ko', ms=3, zorder=3)
        self.excluded2,  = self.inspection_figure_ax2.plot(self.time_array[np.where(self.skip_array)], self.psf_array[np.where(self.skip_array)], 'ro', ms=3, zorder=4)
        self.lowq2, = self.inspection_figure_ax2.plot(
            self.time_array[self.worst_quality_mask], self.psf_array[self.worst_quality_mask],
            's', ms=6, linestyle='None', color='red', markerfacecolor='none', markeredgewidth=1.8,
        ...
        self.setup_window([
            ...
            [[self.Label(text='Double-click on a point to see the frame on the left panel.\n'
                             'Red squares = worst 10% frames by composite quality (see quality_criteria.txt).'), 1, 4]],
            [],
    ...
    def _build_quality_criteria_file_and_worst_mask(self):
        """
        Critères inspirés des usages « stacking » / ASTAP (documentation HNSKY) : niveau de fond,
        dispersion du fond (bruit), largeur PSF (proxy seeing / netteté type FWHM/HFD), masse d'air,
        SNR grossier (moyenne/sigma sur le cadre). Score composite = somme de z-scores robustes
        (plus grand = plus mauvais). Écrit quality_criteria.txt dans le dossier de réduction, trié pire → meilleur.
        """
        n = len(self.science_files)
        if n == 0:
            self.worst_quality_mask = np.array([], dtype=bool)
            self.quality_composite = np.array([])
            return

        mean_cnt = np.array([self.all_frames[f][self.log.mean_key] for f in self.science_files], dtype=float)
        std_cnt = np.array([self.all_frames[f][self.log.std_key] for f in self.science_files], dtype=float)
        with np.errstate(divide='ignore', invalid='ignore'):
            snr_proxy = np.where(std_cnt > 1e-30, mean_cnt / std_cnt, 0.0)

        z_sky = _robust_z(self.sky_mean_array, invert=False)
        z_sig = _robust_z(self.sky_std_array, invert=False)
        z_psf = _robust_z(self.psf_array, invert=False)
        z_am = _robust_z(self.airmass_array, invert=False)
        z_snr = _robust_z(snr_proxy, invert=True)

        self.quality_composite = z_sky + z_sig + z_psf + 0.5 * z_am + z_snr

        order = np.argsort(-self.quality_composite)
        k_worst = max(1, int(np.ceil(0.10 * n)))
        self.worst_quality_mask = np.zeros(n, dtype=bool)
        self.worst_quality_mask[order[:k_worst]] = True

        dir_param = self.log.get_param('directory')
        if dir_param and dir_param != 'Choose Directory':
            work_dir = os.path.join(dir_param, self.log.reduction_directory)
        else:
            work_dir = self.log.reduction_directory
        qpath = os.path.join(work_dir, 'quality_criteria.txt')

        psf_fwhm_equiv = self.psf_array * 2.0

        lines = []
        lines.append('# Quality criteria (NPOAP/HOPS Inspection) — inspired by stacking-tool practice')
        lines.append('# (ASTAP / HNSKY: background level, noise, star width ~HFD/FWHM-like, sharpness via PSF).')
        lines.append('# sky_* = ADU/pix/s ; psf_hwhm_pix = HOPS PSF HWHM (pix) ; psf_fwhm_equiv_pix ≈ 2×HWHM (FWHM sym.)')
        lines.append('# snr_proxy_mean_over_sigma = frame mean / std (pixels) ; composite_score higher = worse')
        lines.append('# rank_worst_first : 1 = poorest frame. Sorted worst → best.')
        lines.append('image\tsky_adu_per_pix_s\tsky_sigma_adu_per_pix_s\tpsf_hwhm_pix\tpsf_fwhm_equiv_pix\t'
                      'airmass\tsnr_proxy_mean_over_sigma\tcomposite_score\trank_worst_first')

        for pos, idx in enumerate(order):
            fn = self.science_files[idx]
            lines.append('{0}\t{1:.8g}\t{2:.8g}\t{3:.8g}\t{4:.8g}\t{5:.8g}\t{6:.8g}\t{7:.8g}\t{8:d}'.format(
                fn,
                self.sky_mean_array[idx],
                self.sky_std_array[idx],
                self.psf_array[idx],
                psf_fwhm_equiv[idx],
                self.airmass_array[idx],
                snr_proxy[idx],
                self.quality_composite[idx],
                pos + 1,
            ))
    ...
    def replot_inspection_figure(self):
        ...
        if getattr(self, 'worst_quality_mask', np.array([])).size:
            self.lowq1.set_xdata(self.time_array[self.worst_quality_mask])
            self.lowq1.set_ydata(self.sky_mean_array[self.worst_quality_mask])
            self.lowq2.set_xdata(self.time_array[self.worst_quality_mask])
            self.lowq2.set_ydata(self.psf_array[self.worst_quality_mask])
