...
class HOPSLog:
    ...
    def __init__(self):
        ...
        self.runtime_log_dir = os.path.join(self.__location__, 'runtime_logs')
        self.runtime_log_file = os.path.join(self.runtime_log_dir, 'hops_runtime.log')
        self.runtime_level = 'ERROR'
        self._init_runtime_log()
    ...
    def set_param(self, parameter, value):
        ...
        # Journaliser les changements d'état pour un suivi fin des opérations.
        try:
            self.runtime('set_param: {0}={1}'.format(parameter, value), level='DEBUG')
        except Exception:
            pass
    ...
    def load_local_log(self):
        ...
        self.runtime('Loading local log: {0}'.format(local_log_path))
    ...
    def initiate_local_log(self):
        ...
        local_log_path = os.path.join(self.get_param('directory'), self.files['local_log'])
        self.runtime('Initiating local log: {0}'.format(local_log_path))
    ...
    def save_local_log(self):
        ...
        local_log_path = os.path.join(self.get_param('directory'), self.files['local_log'])
        self.runtime('Saving local log: {0}'.format(local_log_path))
    ...
    def export_local_log(self, path):
        ...
        local_log_path = os.path.join(self.get_param('directory'), self.files['local_log'])
        self.runtime('Exporting local log from {0} to {1}'.format(local_log_path, path))
    ...
    def save_local_log_profile(self, local_log_profile=None):
        ...
        self.runtime('Saved local profile log: {0}'.format(self.files['local_log_profile']))
        self.local_log_profile = self.open_yaml(self.files['local_log_profile'])
    ...
    def save_local_log_user(self):
        ...
        self.runtime('Saved local user log: {0}'.format(self.files['local_log_user']))
    ...
    def open_yaml(self, path):
        self.runtime('YAML read: {0}'.format(path), level='DEBUG')
        return yaml.load(open(path, 'r'), Loader=yaml.SafeLoader)

    def save_yaml(self, data, path):
        self.runtime('YAML write: {0}'.format(path), level='DEBUG')
        yaml.dump(data, open(path, 'w'), default_flow_style=False)

    def _init_runtime_log(self):
        try:
            os.makedirs(self.runtime_log_dir, exist_ok=True)
            with open(self.runtime_log_file, 'a', encoding='utf-8') as w:
                w.write('\n[{0}] [INFO] HOPS runtime log started (v{1})\n'.format(
    ...
    def runtime(self, message, level='INFO'):
        try:
            level_order = {'DEBUG': 10, 'INFO': 20, 'WARNING': 30, 'ERROR': 40, 'CRITICAL': 50}
            current = level_order.get(str(level).upper(), 20)
            minimum = level_order.get(str(self.runtime_level).upper(), 40)
            if current < minimum:
                return
            with open(self.runtime_log_file, 'a', encoding='utf-8') as w:
                w.write('[{0}] [{1}] {2}\n'.format(
    ...
    def runtime_exception(self, context, exc=None):
        try:
            if exc is None:
                details = traceback.format_exc()
            else:
                details = ''.join(traceback.format_exception(type(exc), exc, exc.__traceback__))
            self.runtime('{0}\n{1}'.format(context, details.rstrip()), level='ERROR')
        except Exception:
            pass
